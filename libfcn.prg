...
function Termo(i, Vsego, y1, x1, nClrFon, nClrTerm, nClrPerc, xSound)
  ...
  */
    ...
    ENDIF
    DEFAULT y1 TO 12, x1 TO 20
    @ y1, x1 SAY PADL(ALLTRIM(STR(i/Vsego*100,0)),3,"0") COLOR "+GR/+W"
    */
    if (lOpen)
      if (i > Vsego)
        return (TRUE)
...
function inikop(p1, p2, p3, p4)
  ...
  croptr=alltrim(getfield('t1', 'rtcenr', 'tcen', 'zen'))
  ...
  if (pbzenr=1)
    ...
    cboptr=alltrim(getfield('t1', 'ptcenr', 'tcen', 'zen'))
    if (empty(cboptr))
  ...
  if (fieldpos('pxzen')#0)
    ...
    if (pxzenr=1)
      ...
      cxoptr=alltrim(getfield('t1', 'xtcenr', 'tcen', 'zen'))
      if (empty(cxoptr))
  ...
  sele soper
  for spi=1 to 20
    cddbr='ddb'+alltrim(str(spi, 2))
    cdkrr='dkr'+alltrim(str(spi, 2))
    cprz='prz'+alltrim(str(spi, 2))
    db_r=&cddbr
    kr_r=&cdkrr
...
function SequIdEval(cField, nLenStoreField)
  ...
  do case
  case (UPPER(ALLTRIM(cField))="KDOKK")
    nId:=(VAL(RIGHT(ALLTRIM(STR(YEAR(memvar->gdTd))), 2))*100+MONTH(memvar->gdTd))*10^(nLenStoreField-4)+nId
...
function prskbs(p1, p2, p3, p4, p5)
  ...
  if (netseek('t1', 'd0k1_r,vu_r,vo_r,kop_r'))
    for spi=1 to 20
      ...
      db_rrr=&cddbr
      kr_rrr=&cdkrr
      cdb_rrr=alltrim(str(db_rrr))
      ckr_rrr=alltrim(str(kr_rrr))
  ...
  if (gnVo=9.and.gnCtov=1)
    if (napr#0)
      ...
      if (blkr#0)
        ...
        wmess('Направление '+alltrim(nnapr)+' заблокировано', 2)
        return (.t.)
      endif
