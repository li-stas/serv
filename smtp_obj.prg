...
return obj
****************************************************************
function smtpNew(host,port)
        local obj:=map()

        obj:classname   := "SMTP"
        obj:host        := iif(host==NIL,"localhost",host)
        obj:port        := iif(host==NIL,25,port)
        obj:timeout     := 6000
        obj:EOL         := "&\n"
        obj:LF          := CHR(10)
        obj:handle      := NIL
...
static function smtp_connect()
        ...
        ::handle := tcpconnect(::host,::port,::timeout)
...
static function smtp_send(sFrom,sTo,sData)
        ...
        if !empty(sData)
                ...
                if ret = -1
                        ::error:="Error write DATA:"+ferror()+":"+ferrorstr()
                        return .f.
                elseif ret  # LEN(sData)
                        ::error:="Error write DATA: ret#LEN(sData) "+ltrim(str(ret))+" "+ltrim(str(len(sData)))
                        return .f.
                endif
