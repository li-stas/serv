...
para p1,p2,p3
  ...
  *****************************************
  ...
  erase prs2.dbf
  erase prs2.cdx
  lcrtt('prs2','rs2','f:kg c:n(3) f:nat c:c(60) f:zenbb c:n(15,3) f:zenbt c:n(15,3) f:id c:c(9) f:nai c:c(60) f:vesp c:n(12,3) f:NoPrn c:n(1)')
  sele 0
  use prs2 excl
  ...
  erase prs2m.dbf
  erase prs2m.cdx
  lcrtt('prs2m','rs2','f:kg c:n(3) f:nat c:c(60) f:zenbb c:n(15,3) f:zenbt c:n(15,3) f:id c:c(9) f:nai c:c(60) f:vesp c:n(12,3) f:nei c:c(4) f:NoPrn c:n(1)')
  sele 0
  use prs2m excl
  ...
  if fieldpos('mntov177')#0
    ...
    kolposNoPrn:=0
  // всернутые документы
  if EMPTY(PrnOprnr) .and. (pr177r=2.or.pr169r=2.or.pr129r=2.or.pr139r=2)
  ...
  EndIf
  sele prs2
  inde on STR(kg,3)+nat+str(ktlp,9)+str(ppt)+str(ktl) tag t1
  inde on STR(kg,3)+str(vesp,12,3) tag t2
  set orde to tag t1
  go top
...
if gnVo=9.or.gnVo=2.or.gnVo=1.or.gnVo=3.or.gnVo=10  // Покупатели,магазины
   if p1#3
      ...
      If (who=3.or.who=4)
        ...
        If ChkPrn11tn(.f.); aPrn[1]:=RTrim(aPrn[1])+'+ТТН  ';  EndIf
        vzz=alert('Тип документа',aPrn)
        If vzz=4; lPrn11tn:=.T.; EndIf
      endif
   else
      vzz=1
   endif
...
*****************************************************
   do case
      case aPrnr = 2.or.aPrnr = 3 // Бланк строгой отчетности
        /*
*           if !alltrim(uprlr)=='dadi'
*               wmess('Только Павловна Низзя!!!',2)
*               retu
*           endif
*           if cntrs2r>48
*              save scre to scmess
*              mess('Количество позиций превышает к-во позиций бланка',1)
*              rest scre from scmess
*              exit
*           endif */
           if aPrnr = 2
...
PROCEDURE PrnDoc()
  ...
  if !file('ptara.dbf')
     ...
     inde on str(int(ktl/1000000),3)+nat tag t2
  else
     if select('ptara')#0
        sele ptara
        use
  ...
  Do While ttn=ttnr
    if vzz=1.or.vzz=3 // TTN тип док 1 ТТН 3 Счет
     ...
     IF tov->(Seek_Tov___(Sklr,Ktlr,ttnr))
        ...
        /*
        *        if c27#0
        *           cc27r=str(c27,4,2) // Макс розн цена (Табак)
        *        else
        *           cc27r=''
        *        endif
       */
        ...
        if prUpr=0.or.vzz=2
           ...
           if getfield('t1','int(ktlr/1000000)','sgrp','mark')=1
              // OFF 06-07-17 01:32pm  nat_r+=alltrim(getfield('t1','kger','grpe','nge'))+' '
          ...
          if kkr=1.and.gnEnt=20
            ...
            nat_r=left(nat_r,48) // для усеченной печати длинна должна быть 48
          endif
        endif
        if vzz=1.or.vzz=3 // TTN
           if !empty(nair)
              ...
              if gnEnt=21.and.kplr=3736127
                 nat_r=nat_r+iif(!empty(nsertr),' '+nsertr,'')+' '+iif(barr#0,' '+str(barr,13),'')+uktr()
              else
                 if prUpr=0
                    nat_r=nat_r+iif(!empty(nsertr),' '+nsertr,'')+uktr()
                 else
                    if kkr=1
                    else
                       nat_r=nat_r+iif(!empty(nsertr),' '+nsertr,'')+uktr()
        ...
        if lnat_r<lnn //50
          if upakr=0.or.(vzz=1.or.vzz=3).and.!empty(idr)
            ...
            if gnEnt=21
              if kplr=3736127
                nat_r=iif(.t.,alltrim(getfield('t1','kger','grpe','nge'))+' '+alltrim(natr),alltrim(natr))+iif((vzz=1.or.vzz=3).and.!empty(nsertr),' '+nsertr,'')+' '+iif(barr#0,' '+str(barr,13),'')+uktr()
              else
                nat_r=iif(.t.,alltrim(getfield('t1','kger','grpe','nge'))+' '+alltrim(natr),alltrim(natr))+iif((vzz=1.or.vzz=3).and.!empty(nsertr),' '+nsertr,'')+uktr()
              endif
            else
              if prUpr=0
                nat_r=iif(getfield('t1','int(ktlr/1000000)','sgrp','mark')=1,alltrim(getfield('t1','kger','grpe','nge'))+' '+alltrim(natr),alltrim(natr))+iif((vzz=1.or.vzz=3).and.!empty(nsertr),' '+nsertr,'')+uktr()
              else
                if kkr=1
                else
                  nat_r=iif(getfield('t1','int(ktlr/1000000)','sgrp','mark')=1,alltrim(getfield('t1','kger','grpe','nge'))+' '+alltrim(natr),alltrim(natr))+iif((vzz=1.or.vzz=3).and.!empty(nsertr),' '+nsertr,'')+uktr()
        ...
        if gnEnt=8.and.upakr#0.and.mod(kvpr,1)=0
          ...
          if lnat_r=lnn  //50
            if zcr=0
              if .f. //vzz=3
                ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+;
                nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+;
                str(aaa,3)+' '+str(zenr,9,3)+' '+;
                iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+;
                str(prmnr,6)+' '+dtoc(dizgr)+' '+dtoc(drlzr)+' '+;
                iif(rs2sertr#0,'C','')
              else
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                  if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                     ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(aaa,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+iif(rs2sertr#0,'C','')
                  else
              ...
              if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(aaa,3)+' '+str(opt_r,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+iif(rs2sertr#0,'C','')
                rsle()
             ...
             if !(kopr=177.and.(vzz=1.or.vzz=3))
                if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                   ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+xxx+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(aaa,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+iif(rs2sertr#0,'C','')
                else
        ...
        if len(nat_r)=lnn
          if zcr=0
            if .f. //vzz=3
              ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+str(prmnr,6)+' '+dtoc(dizgr)+' '+dtoc(drlzr)+' '+iif(rs2sertr#0,'C','')
            else
              if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                  if str(gnEnt,3)$' 20; 21'.and.(vzz=1.or.vzz=3).and.prUpr=1
                    ...
                    ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+iif(rs2sertr#0,'C','')
                  endif
                else
                  ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)
                endif
              else
                if !(kopr=177.and.(vzz=1.or.vzz=3))
            ...
            if .f. //vzz=3
              ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(opt_r,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+str(prmnr,6)+' '+dtoc(dizgr)+' '+dtoc(drlzr)+' '+iif(rs2sertr#0,'C','')
            else
              if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                  ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(opt_r,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+iif(vzz=2.and.ktlmr=0.and.int(ktlr/1000000)=507,'*','')+' '+iif(rs2sertr#0,'C','')
                else
                  ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+nat_r+' '+subs(neir,1,4)+' '+str(kvpr,10,3)
                endif
              else
           ...
           if .f. //vzz=3
              ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+xxx+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+' '+str(prmnr,6)+' '+dtoc(dizgr)+' '+dtoc(drlzr)+' '+iif(rs2sertr#0,'C','')
           else
             if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
               if !(kopr=177.and.vzz=1)
                  ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+xxx+' '+subs(neir,1,4)+' '+str(kvpr,10,3)+' '+str(zenr,9,3)+' '+iif(svpr<10000000,str(svpr,9,2),str(svpr,12,2))+iif(vzz=2.and.ktlmr=0.and.int(ktlr/1000000)=507,'*','')+' '+iif(rs2sertr#0,'C','')
               else
                 ?str(ktlr,9)+iif(locr#0,str(locr,6),'')+' '+xxx+' '+subs(neir,1,4)+' '+str(kvpr,10,3)
               endif
             else
               if !(kopr=177.and.(vzz=1.or.vzz=3))
        ...
        /*
          lSeekError:=.T.
          #ifdef __CLIP__
            outlog(__FILE__,__LINE__,"SEEK_TOV___",cKeySeek,LEN(cKeySeek),sklr,ktlr)
          #endif
        */
        ?str(ktlr,9)+' Не найден в TOV'
        kolpos_r=kolpos_r+1
        rsle()
     endif
     ...
     IF vzz#3.and.!(kopr=177.and.(vzz=1.or.vzz=3))
        IF kecsr#0
           if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
              if rs1->prZZen=0
                 if .t. //gnEnt=20 *01
                    if kopr=169.or.gnEntRm=1.and.svczr=0
 //                    if kopr=169.or.svczr=0
                       ?''
                    else
  ...
  do case
     case (gnVo=9.and.gnArm#6).or.gnVo=2.or.gnVo=6.or.gnVo=1.or.gnVo=8.or.gnVo=3 // Покупатели,магазины,переброска,возвр.пост.
          If Who = 2.or.who=5 .Or. ((Who = 3.or.who=4) .And. Vzz = 2)
             ...
             ?' "___" ____________ 19   г. Вага (бруто)  '+str(svesr,10,3)+' кг Кiл.мiсць '+str(vmestr,5)
          EndIf
             ...
             if kopr#168.or.kopr=168.and.((vzz=1.or.vzz=3).and.kkr#2.or.vzz#1)
                ...
                do case
                   case gnEnt=20 //.and.gnSk=228
                        nvipdr=subs(getfield('t1','60','speng','fio'),1,15)
                ...
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                   if rs1->prZZen=0
                      if gcPath_m='i:\pl\'
                         ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ iif(!empty(dopr),dtoc(dopr),dtoc(dotr))+' г.'+ '          Оператор '+'  ___________ '
                      else
                         if gnEnt=20
                            ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ iif(przr=0,dtoc(dopr),dtoc(dotr))+' г.'+ '          Оператор '+alltrim(gcName)+'  ___________ '
                         else
                            ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ iif(!empty(dopr),dtoc(dopr),dtoc(dotr))+' г.'+ '          Оператор '+alltrim(gcName)+'  ___________ '
                         endif
                      endif
                   else
                      if gcPath_m='i:\pl\'
                         ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ dtoc(rs1->dot)+' г.'+ '          Оператор '+' ___________ '
                      else
                         ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ dtoc(rs1->dot)+' г.'+ '          Оператор '+alltrim(gcName)+' ___________ '
                      endif
                   endif
                endif
             endif
   ...
   //           ?''
   //           rsle()
             ?'Вiдпуск пiдтвердив '+nvipdr+' ___________       '+ dtoc(rs1->dop)+' г.'+ '          Оператор '+alltrim(gcName)+' ___________ '
             rsle()
             ?'Вантаж до перевезення прийняв  ___________                           Вантаж одержав ____________'
             rsle()
          endif
...
func RsSh(p1)
  ...
  if p1=nil
    ...
    case gnVo=9.or.gnVo=2.or.gnVo=1.or.gnVo=3.or.gnVo=10
      if !(kopr=177.and.(vzz=1.or.vzz=3))
        ...
        if brprr=0
            ...
            ?alltrim(Nbankr)+' МФО-'+str(Bankr,6)+' р/с-'+Schtr
            rsle()
        endif
        ...
        rsle()
          ...
          if gnEnt=20.and.(vzz=1.or.vzz=3)
            ...
            if getfield('t1','ktar','s_tag','ktakkl')#0
                if przr=0
                  if rs1->prZZen=0
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                  else
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)
                  endif
                else
                  if rs1->prZZen=1.and.(gnEnt=20.or.gnEnt=21) // *01
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dotr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)
                  else
                      if gnEnt=20
                        ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dotr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                      else
                        ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dopr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                      endif
                  endif
                endif
            else
                if przr=0
                  if rs1->prZZen=0
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktasr#0,'Тор.агент  '+getfield('t1','ktasr','s_tag','fio'),'')
                  else
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)
                  endif
                else
                  if rs1->prZZen=1.and.(gnEnt=20.or.gnEnt=21) // *01
                      ...
                      if gnEnt=20
                        ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dotr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktasr#0,'Тор.агент  '+getfield('t1','ktasr','s_tag','fio'),'')
                      else
                        ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dopr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fkto_r)+'  '+iif(ktasr#0,'Тор.агент  '+getfield('t1','ktasr','s_tag','fio'),'')
                      endif
                  endif
                endif
            endif
            ...
            if przr=0
                if rs1->prZZen=0
                  ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                else
                  ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dvpr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)
                endif
            else
                if rs1->prZZen=1.and.(gnEnt=20.or.gnEnt=21) // *01
                  ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dotr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)
                else
                  if gnEnt=20
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dotr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                  else
                      ?'Склад джерело  '+alltrim(gcNskl)+' от '+dtoc(dopr)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)+'  '+iif(ktar#0,'Тор.агент  '+getfield('t1','ktar','s_tag','fio'),'')
                  endif
                endif
            endif
          endif
    ...
    case gnVo=6
          if sktr#0
            ?'Склад получатель '+getfield('t1','sktr','cskl','nskl')+' '+AtNomr
            rsle()
          endif
          ?'Склад джерело  '+alltrim(gcNskl)+' Товарознавець '+iif(gcPath_m='i:\pl\','',fktor)
          rsle()
    case gnVo=7
          ?'Склад получатель '+getfield('t1','sktr','cskl','nskl')
          rsle()
     ...
     if gnVo#4
      //!!
        if kplr#3210425
          ...
          if fieldpos('cndog')=0
             if gcPath_m='i:\pl\'
                ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+str(getfield('t1','kplr','klndog','ndog'),6)+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))
             else
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                   ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+str(getfield('t1','kplr','klndog','ndog'),6)+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))+iif(ttnpr#0,' '+str(ttnpr,6),'')+iif(ttncr#0,' '+str(ttncr,6),'')
                else
             ...
             if empty(getfield('t1','kplr','klndog','cndog'))
                ...
                if gnEnt=20.and.kplr=3229492.and.prmk17r=1.and.!(kopr=177.and.(vzz=1.or.vzz=3))
                   if gcPath_m='i:\pl\'
                      ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+'383/КО/11-ПН'+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))
                   else
                      if !(kopr=177.and.(vzz=1.or.vzz=3))
                         ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+'383/КО/11-ПН'+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))+iif(ttnpr#0,' '+str(ttnpr,6),'')+iif(ttncr#0,' '+str(ttncr,6),'')
                      else
                         ?'Код опеpацiї - '+str(kopr,3)
                      endif
                   endif
                   ...
                   if gcPath_m='i:\pl\'
                      ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+alltrim(getfield('t1','kplr','klndog','cndog'))+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))
                   else
                      if !(kopr=177.and.(vzz=1.or.vzz=3))
                         ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' Договiр '+alltrim(getfield('t1','kplr','klndog','cndog'))+' от '+dtoc(getfield('t1','kplr','klndog','dtdogb'))+iif(ttnpr#0,' '+str(ttnpr,6),'')+iif(ttncr#0,' '+str(ttncr,6),'')
                      else
                         ?'Код опеpацiї - '+str(kopr,3)
                      endif
                   endif
          ...
          if fieldpos('cndog')=0
             if gcPath_m='i:\pl\'
                ...
                if !(kopr=177.and.(vzz=1.or.vzz=3))
                   ?'Код опеpацiї - '+str(kopr,3)+' '+nopr+' '+iif(ttnpr#0,' '+str(ttnpr,6),'')+iif(ttncr#0,' '+str(ttncr,6),'')
                else
                   ?'Код опеpацiї - '+str(kopr,3)
                endif
             endif
  ...
  *if vzz#3
     if kopr#168
        if gnEnt=21.and.vor=9.and.nkklr#0.and.nkklr#20034.and.p1=nil.and..f.
           ...
           rsle()
           sele cskl
           loca for ent=gnEnt.and.tpstpok=2
           if foun()
...
func prnlvz()
  ...
  *??chr(27)+'E'+chr(27)+'&l4h25a0O'+chr(27)+'&k2S'+chr(27)+'(3R'+chr(27)+'(s0p18.00h0s1b4102T'+chr(27)
  ...
  do while !eof()
     ...
     smpr=smpr+svpr
     if len(natr)<=34
        ?' '+str(ktlr,9)+' '+natr+'  '+str(zen20r,10,2)+'   '+str(zenr,10,2)+space(4)+'  '+str(kvpr,5)+' '+str(svpr,9,2)
     else
        ?' '+str(ktlr,9)+' '+subs(natr,1,34)+'  '+str(zen20r,10,2)+'   '+str(zenr,10,2)+space(4)+'  '+str(kvpr,5)+' '+str(svpr,9,2)
        ?''
